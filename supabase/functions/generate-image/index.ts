...
serve(async (req) => {
  ...
  try {
    ...
    logStep('Generating image with Gemini', { prompt: enhancedPrompt.substring(0, 100), style, aspectRatio });
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        throw new Error('Créditos insuficientes na API. Entre em contato com o suporte.');
      }
      throw new Error(`Erro na geração: ${response.status}`);
    }

    const data = await response.json();
    logStep('Gemini response received', { hasChoices: !!data.choices });
