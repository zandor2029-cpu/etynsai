    ...
    <div className="space-y-3">
        ...
        {images.length > 0 && (
          ...
          >
            {images.map((url, index) => (
              ...
              >
                <div className="w-24 h-24 rounded-xl overflow-hidden border-2 border-watermelon-pink/30 bg-background/50">
                  <img
                    src={url}
                ...
                <button
                  ...
                  className="absolute -top-2 -right-2 p-1.5 rounded-full bg-destructive text-destructive-foreground opacity-0 group-hover:opacity-100 transition-opacity shadow-lg hover:bg-destructive/90"
                >
                  <X className="w-3 h-3" />
                </button>
                <div className="absolute bottom-1 left-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-background/80 text-foreground">
                  {index + 1}
                </div>
              </motion.div>
            ))}
          ...
          <div className="flex flex-col items-center gap-2 text-center">
            {isUploading ? (
              <>
                ...
                >
                  <Upload className="w-6 h-6 text-watermelon-pink" />
                </motion.div>
              ...
              <>
                <div className="flex items-center gap-2">
                  <Image className="w-5 h-5 text-watermelon-pink" />
                  <Upload className="w-5 h-5 text-watermelon-green" />
                </div>
      ...
      {images.length > 0 && (
        <motion.div
          ...
          className="flex items-start gap-2 p-3 rounded-lg bg-watermelon-green/10 border border-watermelon-green/20"
        >
          <AlertCircle className="w-4 h-4 text-watermelon-green mt-0.5 flex-shrink-0" />
          <div className="text-xs text-muted-foreground">
            <strong className="text-foreground">Dica:</strong> Descreva no prompt como combinar as imagens. 
            Ex: "Pegue a pessoa da imagem 1 e coloque no cenário da imagem 2" ou 
            "Mescle os estilos das duas imagens".
          </div>
        </motion.div>
      )}
    </div>
